...
use humhub\modules\space\models\Gallery;
...
/*
    protected function getGallery()
    {
        ...
        $gid = Yii::$space->request->get('gid');

        if(!$gid) {
            throw new BadRequestHttpException();
        }

        $this->gallery = Gallery::find()->contentContainer($this->contentContainerController)->where(['gallery_gallery.id' => $gid])->one();
...
/**The code is edited by Hina */
    public function actionGallery()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->upload->saveAs('upload/'.$fileName);
